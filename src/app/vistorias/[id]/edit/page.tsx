...
import { useInspection, useUpdateInspection } from '@/hooks/useApi';
...
export default function EditVistoriaPage() {
  ...
  const { data: inspection, isLoading, error: inspectionError } = useInspection(inspectionId);
  ...
  useEffect(() => {
    if (inspection) {
      ...
      setChecklistItems(inspection.items);
    }
  }, [inspection]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Erro ao atualizar vistoria');
    }
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600">Modificar dados da inspeção veicular</p>
          </div>
        </div>
